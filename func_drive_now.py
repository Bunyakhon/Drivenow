                    ...
                    try:
                        ...
                        for index, record in enumerate(records):
                            if record[0] == edit_id:
                                ...
                                new_name = input(f"ชื่อใหม่ (กด Enter เพื่อไม่แก้ไข): ").encode('utf-8') or record[1]
                                print(f"รถรุ่น: {record[2].decode().strip()}")
                                new_model = input(f"แบบรถใหม่ (กด Enter เพื่อไม่แก้ไข): ") or record[2].decode().strip('\x00')
                                ...
                                new_type = car_type.encode('utf-8')
                                print(f"วันที่เช่าเดิม: {record[4].decode().strip()}")
                                new_rent_date = input(f"วันที่เช่าใหม่ (กด Enter เพื่อไม่แก้ไข): ").encode('utf-8') or record[4]
                                print(f"วันที่คืนรถ: {record[5].decode().strip()}")
                                new_return_date = input(f"วันที่คืนใหม่ (กด Enter เพื่อไม่แก้ไข): ").encode('utf-8') or record[5]
                                print(f"ราคาเดิม: {record[6]:.2f}")
                                new_cost = float(input(f"จำนวนเงินใหม่ (กด Enter เพื่อไม่แก้ไข): ") or record[6])

                                updated_record = (
                                    record[0],
                                    new_name,
                                    new_model.encode('utf-8'),
                                    new_type,
                                    new_rent_date,
                                    new_return_date,
                                    new_cost
                                ...
                                records[index] = updated_record  # อัปเดตเรคคอร์ดในลิสต์ที่ถูกต้อง
                                break
