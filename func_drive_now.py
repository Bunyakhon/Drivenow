            ...
            while keep_going:  # ลูปนี้จะแสดงเมนูเช่ารถจนกว่าจะเลือกออก
                ...
                if rent_car_menu == 1:  # ถ้าผู้ใช้เลือกตัวเลือกที่ 1 (แสดงรายการรถที่มีให้เช่า)
                    ...
                    return {  # ส่งคืนข้อมูลการเช่ารถในรูปแบบดิกชันนารี
                        ...
                        "จำนวนเงิน": total_cost_rent,  # จำนวนเงินรวมที่ต้องจ่าย
                ...
                if edit_car_menu == 1:  # ถ้าผู้ใช้เลือกตัวเลือกที่ 1 (แสดงข้อมูลทั้งหมด)
                        ...
                        with open("Doc/datas_drive_now.bin", "rb") as file:  # เปิดไฟล์ไบนารีเพื่ออ่านข้อมูล
                            ...
                            while True:
                                ...
                                if record[0] == edit_id:  # ตรวจสอบว่าหมายเลขลำดับตรงกับที่ผู้ใช้ระบุหรือไม่
                                    ...
                                    new_name = input("กรุณากรอกชื่อใหม่: ").encode('utf-8')  # รับชื่อใหม่และแปลงเป็นไบนารี (utf-8)
                                    new_model = input("กรุณากรอกแบบรถใหม่: ")  # รับรุ่นรถใหม่จากผู้ใช้
                                    ...
                                    new_rent_date = input("กรุณากรอกวันที่เช่าใหม่ (YYYY/MM/DD): ").encode('utf-8')  # รับวันที่เช่าใหม่จากผู้ใช้
                                    new_return_date = input("กรุณากรอกวันที่คืนใหม่ (YYYY/MM/DD): ").encode('utf-8')  # รับวันที่คืนใหม่จากผู้ใช้
                                    new_cost = float(input("กรุณากรอกจำนวนเงินใหม่: "))  # รับค่าใช้จ่ายใหม่จากผู้ใช้
                    ...
                    try:
                        with open("Doc/datas_drive_now.bin", "wb") as file:  # เปิดไฟล์ในโหมดเขียน (wb)
                            for rec in records:  # เขียนข้อมูลเรคคอร์ดทั้งหมดที่อัปเดตแล้วกลับลงไฟล์
                                file.write(struct.pack("i100s50s50s15s15sf", *rec))  # เขียนข้อมูลในฟอร์แมตไบนารีลงไฟล์
                            print("ข้อมูลได้รับการอัปเดตเรียบร้อยแล้ว")  # แสดงข้อความยืนยันว่าข้อมูลได้รับการอัปเดตแล้ว
                    
                    except Exception as e:  # จัดการข้อผิดพลาดในการบันทึกข้อมูล
                        print(f"เกิดข้อผิดพลาดในการบันทึกข้อมูล: {e}")
                
                elif edit_car_menu == 3:  # ถ้าผู้ใช้เลือกตัวเลือกที่ 3 (ออกจากเมนูการเช่ารถ)
                    print("ออกจากเมนูการเช่ารถ")  # แสดงข้อความว่าออกจากเมนู
                    keep_going = False  # กำหนดให้ลูปหยุดการทำงานเพื่อออกจากเมนู

                else:  # กรณีที่ผู้ใช้เลือกหมายเลขเมนูไม่ถูกต้อง
                    print("เมนูที่เลือกไม่ถูกต้อง")  # แสดงข้อความเตือนว่าหมายเลขเมนูไม่ถูกต้อง

        except Exception as e:  # จัดการข้อผิดพลาดทั่วไปในฟังก์ชัน
